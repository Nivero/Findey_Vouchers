...
namespace FindeyVouchers.Services
{
    public class MailService : IMailService
    {
        ...
        public async Task<Response> SendMail(string receiverAddress, string subject, string body, List<SendgridMailAttachment> files)
        {
            var client = new SendGridClient(_apiKey);
            var from = new EmailAddress(_senderEmail, _senderName);
            var to = new EmailAddress(receiverAddress);
            ...
            {
                msg.AddAttachment(file.Name, file.Image);
            }
            var response = await client.SendEmailAsync(msg);
            return response;
        }
